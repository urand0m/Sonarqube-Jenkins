    ...
    stages {
        ...
        stage('Dynamic Analysis - Arachni'){
            steps {
                ...
                sh '/opt/arachni/bin/arachni --checks=*,-code_injection_php_input_wrapper,-ldap_injection,-no_sql*,-backup_files,-backup_directories,-captcha,-cvs_svn_users,-credit_card,-ssn,-localstart_asp,-webdav --timeout=00:20:00 --http-user-agent=\'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\' --report-save-path=scan_report_$(date -d "today" +"%Y%m%d%H%M").afr http://dockerd:8080/JavaVulnerableLab/'
