        ...
        stage('Dynamic Analysis - Arachni'){
            steps {
                timeout(time: 1, unit: 'HOURS') {
                    ...
                    sh '/opt/arachni/bin/arachni_reporter scan_report_$today.afr --reporter=html:outfile=scan_report_$today.zip ; '

                }
                println ${ARACHNI_REPORT}
                archiveArtifacts artifacts: 'dependency-check-report.html',onlyIfSuccessful: true
            }
        }
